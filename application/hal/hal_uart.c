...
static const uint16_t uart_len_max[UART_GROUP_CNT] = {UART0_BUF_LEN, UART1_BUF_LEN};
static volatile uint8_t *pt[UART_GROUP_CNT] = {uart0_buf, uart1_buf};
...
void hal_uart_init(Uart_Group_t uart_group, uint32_t baudrate, uart_callback_t callback)
{
    if (UART_0 == uart_group)
    {
        UART0_TX_GPIO->DATA |= UART0_TX_PIN;
    ...
    {
        UART1_TX_GPIO->DATA |= UART1_TX_PIN;
...
void hal_uart_sleep(Uart_Group_t uart_group, uint8_t mode, uint8_t level)
{
    if (UART_0 == uart_group)
    {
        ...
        GPIO_SetPinMFType(UART0_TX_GPIO, UART0_TX_PIN, GPIO_MF_TYPE_GPIO, mode);
        GPIO_SetPinMFType(UART0_RX_GPIO, UART0_RX_PIN, GPIO_MF_TYPE_GPIO, mode);
        UART_DisableInt(OB_UART0, UART_INT_RBR);
        NVIC_DisableIRQ(UART0_IRQn);
    }
    else if (UART_1 == uart_group)
    {
        ...
        GPIO_SetPinMFType(UART1_TX_GPIO, UART1_TX_PIN, GPIO_MF_TYPE_GPIO, mode);
        GPIO_SetPinMFType(UART1_RX_GPIO, UART1_RX_PIN, GPIO_MF_TYPE_GPIO, mode);
        UART_DisableInt(OB_UART1, UART_INT_RBR);
        NVIC_DisableIRQ(UART1_IRQn);
    }
}
