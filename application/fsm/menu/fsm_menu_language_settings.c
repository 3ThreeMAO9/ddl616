...
/***************Function***************/
static QState menu_system_language_event_handle(LockFsm *me, QEvent const *e)
{
    ...
    case KEY_CAN:
        hmiTaskSetState(HMI_STATE_KEY_BOARD_PRESS);
        state = Q_TRAN(lock_fsm_menu_system_settings);
        break;
    default:
        break;
    }
...
QState lock_fsm_menu_language_settings(LockFsm *me, QEvent const *e)
{
    ...
    OB_LOGD(TAG, "Fsm_state[%s], Event[%u, %u]", "language_settings", e->sig, e->dynamic_[0]);
    ...
    switch (e->sig){
        case Q_ENTRY_SIG:
            ...
            hmiTaskSetState(HMI_STATE_LANGAGESETTING);
#if (Enabled == PRINTF_FSM)
            OB_LOGI(TAG, "case KEY_NUM_1:    //中文请按1");
            OB_LOGI(TAG, "case KEY_NUM_2:    //英文请按2");
#endif
            break;
        case Q_EXIT_SIG:
            break;
